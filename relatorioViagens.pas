...
const SQLAgendaTotal  = 'SELECT '+
                        'SUM(CUSTOTOTALKM) AS CUSTO_TOTAL, ID_DOCENTE '+
                        'FROM AGENDA ';
...
type
  TfmRelatorioViagens = class(TForm)
    ...
    frRelatorioViagemAnalitico: TfrxReport;
    ...
    frRelatorioViagemSintetico: TfrxReport;
...
begin
  try
    ...
    //if (cdsCadGeral.IsEmpty) or (cdsAgenda.IsEmpty) or (cdsAgendaDetail.IsEmpty) then
    ...
    frRelatorioViagemAnalitico.Variables.Variables['titulo'] := QuotedStr(LowerCase(msg));
    frRelatorioViagemSintetico.Variables.Variables['titulo'] := QuotedStr(LowerCase(msg));

    if Question('Deseja imprimir relatório detalhado?') then
      frRelatorioViagemAnalitico.ShowReport(True)
    else
      frRelatorioViagemSintetico.ShowReport(True);
  finally
    cdsAgendaDetail.Active  := False;
    cdsAgendaTotal.Active   := False;
    cdsAgenda.Active  := False;
...
begin
  ...
  if qu > '' then
    Result := LowerCase(' WHERE '+qu+' ORDER BY C.NOME ')
  else
    Result := LowerCase(' ORDER BY C.NOME');
...
begin
  ...
  if Trim(qu) > '' then
    Result := LowerCase(' WHERE '+qu+ ' GROUP BY ID_DOCENTE, KMSAIDA, KMRETORNO ORDER BY DATAAGENDAINI, DATAAGENDAFIM ')
  else
    Result := LowerCase(' GROUP BY ID_DOCENTE, KMSAIDA, KMRETORNO ORDER BY DATAAGENDAINI, DATAAGENDAFIM ');
end;
...
begin
  ...
  if Trim(qu) > '' then
    Result := LowerCase(' WHERE '+qu+' GROUP BY ID_DOCENTE ORDER BY ID_DOCENTE ')
  else
    Result := LowerCase(' GROUP BY ID_DOCENTE ORDER BY ID_DOCENTE ');
end;
